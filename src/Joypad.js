...
var Joypad = cc.Layer.extend({
	...
	init:function(){
		...
		if (this._super()){
			...
			this.addChild(cc.Sprite.create(res.s_Joypad1));
			this.addChild(cc.Sprite.create(res.s_Joypad2));
			this._pControlSprite = cc.Sprite.create(res.s_Joypad3);
...
//		cc.registerTargetedDelegate(0, true, this);
        ...
        cc.eventManager.addListener({event: cc.EventListener.TOUCH_ONE_BY_ONE,
            onTouchBegan:function (touch, event){
                ...
                joypad.updateRotation();
                if(joypad._pDelegate)
                    joypad._pDelegate.actionJoypadStart(joypad._pRotation);
                else
            ...
            onTouchMoved:function (touch, event){
                ...
                if (joypad._pDelegate)
                    joypad._pDelegate.actionJoypadUpdate(joypad._pRotation);
                else
                    cc.log('_pDelegate is null ... ');
            ...
            onTouchEnded:function (touch, event){
                ...
                joypad.updateRotation();
                if (joypad._pDelegate)
                    joypad._pDelegate.actionJoypadEnded(joypad._pRotation);
                else
                    cc.log('_pDelegate is null ... ');
            },
            updateTouchRotation:function(touch, event){
                var curPoint = touch.getLocation();
                var sp = cc.pSub(curPoint, joypad._pDefaultPoint);
                var angle = cc.pToAngle(sp) ;// * -57.29577951;
                var rotation = angle * -57.29577951;
                rotation = rotation < 0 ? 360 + rotation: rotation;
                joypad._pRotation = rotation;
            }},this);
	},
	onExit:function(){
